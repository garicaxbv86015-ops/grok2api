...
attemptLoop:
	...
	handleResponse:
		...
		// 是否已是最终出口 403：首次可重试时不当作最终失败，避免过早惩罚。
		finalEgressForbidden := egressForbidden && (attempt > 0 || !attemptPolicy.hasNext(attempt))
		// 先分类 403 正文：明确封禁会使账号失效并换号；请求级安全拒绝原样返回且无副作用；
		// 其余 403 走出口重试路径，不惩罚账号。x-should-retry:false 不能阻止账号级冷却与换号。
		if response.StatusCode == http.StatusForbidden {
			...
			if isTerminalRequestForbidden(credential.Provider, lastFailure) {
				// 请求级/未知范围 403：还原正文并直接返回，不做 OAuth 刷新、换号、冷却或失效。
