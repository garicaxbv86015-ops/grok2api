...
func (m *Manager) AcquireCredential(ctx context.Context, scope domain.Scope, credential accountdomain.Credential) (*Lease, error) {
	...
	// 1. 逻辑账号组绑定固定代理时走严格租约，Cookie 需在本路径解密（受 FlareSolverr 托管策略约束）。
	if credential.ProxyID != nil {
		credentialCookies := ""
		managedClearance := isGrokWebScope(scope) && m.clearanceMode() == "flaresolverr"
		if !managedClearance && scope != domain.ScopeBuild && strings.TrimSpace(credential.EncryptedCloudflareCookie) != "" {
			decryptedCookies, decryptErr := m.cipher.Decrypt(credential.EncryptedCloudflareCookie)
			if decryptErr != nil {
				return nil, decryptErr
			}
	...
	// 2. 共享出口池路径传入加密 Cookie，由 acquire 内部统一解密与托管策略处理。
...
func (m *Manager) acquireBoundProxy(ctx context.Context, scope domain.Scope, credential accountdomain.Credential, credentialCookies string) (*Lease, error) {
	...
	m.incrementInflight(internalNodeID)
	...
	return &Lease{NodeID: internalNodeID, NodeName: credential.ProxyName, Scope: scope, ProxyURL: proxyURL, UserAgent: userAgent, CFCookies: credentialCookies, client: client.client, browser: client.browser, sticky: sticky, release: func() {
		once.Do(func() {
			m.decrementInflight(internalNodeID)
		})
	}}, nil
}
