...
func (r *AccountRepository) LinkWebToBuild(ctx context.Context, webAccountID, buildAccountID uint64) error {
	...
	err := r.db.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		previousBuildFamilyID := buildAccount.FamilyID
		if err := tx.Model(&accountModel{}).Where("id = ?", buildAccountID).Update("family_id", *webAccount.FamilyID).Error; err != nil {
			return err
		}
		// Build 迁入 Web 账号组后，回收原账号组（若已无任何 Provider 成员）。
		if previousBuildFamilyID != nil && *previousBuildFamilyID != 0 && *previousBuildFamilyID != *webAccount.FamilyID {
			if err := cleanupEmptyAccountFamilies(tx, []uint64{*previousBuildFamilyID}); err != nil {
				return err
			}
		}
...
// currentWebTermsAcceptedAt 仅在条款版本仍有效时返回接受时间，否则视为未接受。
// 参数 value 为最近接受时间，version 为当时接受的条款版本；返回可展示的接受时间或 nil。
...
// linkedWebEgressIdentity 优先使用已持久化的出口身份，缺失时从 Web sourceKey 推导。
// 参数 stored 为库中已存身份，sourceKey 为 Web 账号来源键；返回稳定出口身份字符串。
...
func (r *AccountRepository) deleteAccounts(ctx context.Context, ids []uint64) (int64, error) {
	...
	err := r.db.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		var familyIDs []uint64
		if err := tx.Model(&accountModel{}).Where("id IN ? AND family_id IS NOT NULL", lockedIDs).Pluck("family_id", &familyIDs).Error; err != nil {
			return err
		}
		...
		return cleanupEmptyAccountFamilies(tx, familyIDs)
	})
...
// cleanupEmptyAccountFamilies 删除已不含任何 Provider 凭据的逻辑账号组。
// 参数 db 为事务内数据库句柄，familyIDs 为待检查的账号组标识；返回清理过程中的错误。
func cleanupEmptyAccountFamilies(db *gorm.DB, familyIDs []uint64) error {
	for _, familyID := range familyIDs {
		if familyID == 0 {
			continue
		}
		var remaining int64
		if err := db.Model(&accountModel{}).Where("family_id = ?", familyID).Count(&remaining).Error; err != nil {
			return err
		}
		if remaining == 0 {
			if err := db.Delete(&accountFamilyModel{}, familyID).Error; err != nil {
				return err
			}
		}
	}
...
func (r *AccountRepository) DeleteAccountStatusBatch(ctx context.Context, providerValue account.Provider, status string, now time.Time, limit int) ([]uint64, int, error) {
	...
	err := r.db.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		var familyIDs []uint64
		if err := tx.Model(&accountModel{}).Where("id IN ? AND family_id IS NOT NULL", candidates).Pluck("family_id", &familyIDs).Error; err != nil {
			return err
		}
