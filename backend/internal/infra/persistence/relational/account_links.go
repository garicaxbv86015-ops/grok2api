...
func deleteLinkedGroupsTx(tx *gorm.DB, providerValue account.Provider, lockedRoots []uint64, targets []account.Provider, skipMedia bool) (repository.LinkedDeleteOutcome, error) {
	...
	// 4) 删除前记录所属逻辑账号组，删除后回收已无成员的空组。
	var familyIDs []uint64
	if err := tx.Model(&accountModel{}).Where("id IN ? AND family_id IS NOT NULL", deletable).Pluck("family_id", &familyIDs).Error; err != nil {
		return outcome, err
	}
	...
	if err := cleanupEmptyAccountFamilies(tx, familyIDs); err != nil {
		return outcome, err
	}
	return outcome, nil
}
